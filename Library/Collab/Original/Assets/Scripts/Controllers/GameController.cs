        ...
        if (GetOnlineMode())
        {
            if (syncFromSimulationToScorbot)
            {
                ...
                foreach (string t in targetControl.GetNames())
                    commandControl.SyncSimulationToScorbot(robot, targetControl.GetTarget(t));
                */
            }
            else
            {
                foreach (string t in targetControl.GetNames())
                    commandControl.SyncScorbotToSimulation(robot, targetControl.GetTarget(t));
            }
        }
    }
